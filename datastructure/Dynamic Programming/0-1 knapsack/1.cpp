...
int capacity,n;
void printKnapsackObjects(int i, int w) {
    if (i == 0 || w == 0)
        return;

    if (dp[i][w] != dp[i - 1][w]) {
        
        cout << "Object " << i << " with weight " << weights[i - 1] << " and value " << values[i - 1] << " included" << endl;
        printKnapsackObjects(i - 1, w - weights[i - 1]);
    }
    else {
      
        printKnapsackObjects(i - 1, w);
    }
}
...
printKnapsackObjects(n,capacity);
}